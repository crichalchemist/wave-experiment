"""
Training orchestration for Phi Forecaster via Hugging Face Jobs.

Generates a self-contained training script (with ALL code inline) and
launches it on HF Jobs GPU instances.  The generated script uses the
v2.1-recovery-floors formula: recovery_aware_input() is wired into
compute_phi() so that below-floor constructs get lifted by trajectory
and community capacity before entering the geometric mean.
"""
...
def get_training_script(
    ...
    The script includes PEP 723 inline dependencies, ALL welfare scoring
    ...
    # For generated f-strings that should contain {var}, write {{var}}.
    return f'''\
# /// script
# dependencies = [
#     "torch>=2.0.0",
...
Formula: v{FORMULA_VERSION} (recovery-aware floors wired into compute_phi).
Auto-generated by spaces/maninagarden/training.py.
"""
import math
import os
import json
...
# ============================================================================
# Inline welfare scoring (v{FORMULA_VERSION} — recovery-aware)
# ============================================================================
...
def recovery_aware_input(
    ...
    When below floor, recovery potential lifts the effective value.
    """
    if x_i >= floor_i:
        return x_i
...
def ubuntu_synergy(metrics: Dict[str, float]) -> float:
    """Ubuntu synergy term on RAW metrics."""
    pair_sum = sum(
...
def divergence_penalty(metrics: Dict[str, float]) -> float:
    """Divergence penalty on RAW metrics."""
    sq_sum = sum(
...
def compute_phi(
    ...
    """Compute Phi(humanity) — v{FORMULA_VERSION} with recovery-aware floors."""
    ...
    # Step 1: effective (recovery-aware) values for ALL constructs
    ...
    # Step 2: equity weights on EFFECTIVE values
    weights = equity_weights(effective)

    # Step 3: weighted geometric mean of effective values
    product = 1.0
    for c in ALL_CONSTRUCTS:
...
class PhiForecasterGPU(nn.Module):
    def __init__(self, input_size=36, hidden_size={hidden_size}, n_layers=2, pred_len={pred_len}, dropout=0.1, alpha=0.5):
        super().__init__()
        self.alpha = alpha
...
def main():
    ...
    trackio.init(
        ...
        name=f"v{FORMULA_VERSION}-{{hidden_size}}h-{{epochs}}ep",
        config={{
    ...
    api.upload_file(
        ...
        commit_message=f"PhiForecaster v{FORMULA_VERSION} — val_loss={{best_val_loss:.6f}}",
    )
    # Save metadata
    metadata = {{
    ...
    logger.info(f"Checkpoint + metadata pushed to crichalchemist/phi-forecaster (v{FORMULA_VERSION})")
...
def launch_training_job(
    ...
    if hardware not in HARDWARE_OPTIONS:
        return ("", f"Unknown hardware flavor: {hardware}. Choose from: {list(HARDWARE_OPTIONS.keys())}")
    ...
    try:
        ...
        message = (
            f"Training job launched on {hardware} "
...
def compare_checkpoints(
    ...
    """Load metadata for two revisions and return a comparison dict.

    Returns:
        Dict with keys "a", "b", and "diff". Each revision key maps to
        the training_metadata.json contents. "diff" highlights changed
        numeric fields.
    """
    from model import load_training_metadata

    meta_a = load_training_metadata(repo_id=repo_id, revision=revision_a)
    meta_b = load_training_metadata(repo_id=repo_id, revision=revision_b)
    ...
    return {"a": meta_a, "b": meta_b, "diff": diff}
