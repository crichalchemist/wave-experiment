...
src/inference/       ← n-hop network reasoning, contradiction detection, gap detection pipeline
src/forecasting/     ← Phi trajectory forecasting (ForecastBackbone: CNN1D→LSTM→Attention)
src/data/            ← Epstein dataset loaders, NER entity extraction, knowledge graph
...
**Multi-task loss formula (ADR-028)** — `L_total = L_language + α·L_gap + β·L_assumption` (α=β=0.3). Three output heads: language modeling (vocab_size), gap-type classification (5 types), assumption-type classification (3 types). Each detection track has an independent encoder pathway with `hidden.detach()` to prevent gradient interference with the shared backbone. Custom PyTorch training loop via `MultitaskTrainer` (HF Trainer incompatible with 3-tuple forward).

**A+B+C assumption taxonomy** (integrated into investigation agent's analyze phase via ADR-023; also callable independently as library functions):
...
- **Investigation integration (ADR-023):** `_assumption_scan()` in the agent's analyze phase runs all three modules on each unique document (max 5), logs detections as `Finding(is_assumption_finding=True)`, and generates counter-leads that probe assumptions in the next gather cycle. Budget-aware: Module A falls back to keyword-only when budget < 10, B/C skip when < 5. Counter-lead routing: Module C → `court_listener`/`sec_edgar`; A/B → `web_search`. CLI: `--assumptions/--no-assumptions`, `--assumption-threshold`.
...
**Analysis pipeline gap detection** — `analyze()` in `src/inference/pipeline.py` runs a 4-layer pipeline (parse_intent → retrieve_evidence → fuse_reasoning → verify_inline). Evidence is truncated via `_truncate_evidence()` to fit within a token budget (`_DEFAULT_EVIDENCE_TOKEN_BUDGET=6000` tokens, reserving 2000 for the verify prompt) to prevent vLLM context window overflow. After verification, if confidence exceeds 0.3, `_detect_gaps_inline()` runs a lightweight LLM-assisted gap scan returning up to 3 `Gap` objects (from `src/core/types.py`). Gaps are welfare-scored via `score_gaps_welfare()` when `phi_metrics` is provided. `AnalysisResult.gaps` is a tuple of `Gap` objects, empty by default for backward compatibility.
...
**Entity filter (ADR-016)** — `src/data/entity_filter.py` implements a 3-layer noise filter for ingestion: Layer 1 (junk — FOIA codes, emails, short strings, numeric refs, bracket-redacted), Layer 2 (fuzzy dedup — now delegates to `build_entity_mappings_minhash()` from `dedup.py` for O(n) dedup on large sets, falls back to `difflib.SequenceMatcher` for <500 entities), Layer 3 (role descriptions — possessives, inmate patterns, anonymized officers, role prefixes, lowercase informal). Drops ~22% of raw entities. `DropLog` writes filtered entities to JSONL for investigative audit. Wired into `ingest_epstein()` with `--drop-log` CLI option.
...
**Legal domain gap detection (ADR-007)** — `src/detective/legal_gap_detector.py` detects gaps between law-as-written (STATUTE, REGULATION, CASE_LAW, TREATY, TERRITORIAL) and law-as-applied (ENFORCEMENT_PRACTICE, COMMUNITY_EXPERIENCE) for entities in the knowledge graph. `KnowledgeEdge.legal_domain` is an optional edge-level field. `detect_legal_domain_gaps(graph, entity)` returns `LegalGap` findings (DOCTRINAL or NORMATIVE gap types).

**Person auditor (ADR-027)** — `src/detective/person_auditor.py` decomposes compound claims about a person into atomic claims, verifies each against evidence, and computes severity. `decompose_claims()` uses LLM with sentence-splitting fallback. `verify_claim()` uses LLM with keyword-overlap fallback. `compute_severity()` formula: 40% contradiction ratio + 35% welfare construct coverage + 25% volume. Wired into agent's `_audit_phase()` (top 5 entities from findings) and `InvestigationReport.person_audits`. CLI: `detective audit-person --name "Name" [--data-dir DIR] [--findings TEXT]`.
...
**Working modules:** `src/detective/` (hypothesis, evolution, parallel_evolution, modules A/B/C, experience library), `src/detective/investigation/` (autonomous investigation agent — plan→gather→analyze→reflect→evolve→enrich loop, InvestigationSource Protocol + FOIA/Graph/clearnet adapters (8 sources — ADR-021/022), A+B+C assumption scanning with counter-lead generation (ADR-023), LLM-assisted lead generation, budget enforcement, constitutional halt), `src/core/scoring.py` (shared `parse_score`/`clamp_confidence` used by all detective modules), `src/core/log.py` (centralized logging config — ADR-019), `src/data/sourcing/types.py` (`SourceDocument` frozen dataclass + `DocumentLoader` Protocol — ADR-018), `src/inference/welfare_scoring.py` (Phi v2.1 formula, recovery floors, derivatives, gradient prioritization, curiosity scoring, trajectory urgency — all formula parameters are named constants), `src/inference/welfare_classifier.py` (Hub-first DistilBERT classifier — canonical source for construct scoring, `welfare_scoring.py` wraps with keyword fallback), `src/inference/scenario_extraction.py` (corpus → construct profiles → scenario templates), `src/forecasting/` (PhiTrajectoryForecaster, self-contained ForecastBackbone, pipeline, scenarios), `src/data/sourcing/` (foia_scraper.py with guarded scraping imports, dual_pipeline.py, legal_sources.py, hf_loader.py, doj_loader.py, international_loader.py, ocr_provider.py — all loaders return `SourceDocument` per ADR-018), `src/data/epstein_adapter.py` (epstein-docs parser + entity normalization — ADR-015), `src/data/ingest_epstein.py` (graph population pipeline — ADR-015), `src/data/entity_filter.py` (3-layer entity noise filter + drop logging — ADR-016), `src/data/ner.py` (spaCy NER + heuristic fallback — ADR-025), `src/data/dedup.py` (MinHash/LSH near-duplicate detection + entity mapping bridge — ADR-026), `src/detective/person_auditor.py` (claim decomposition, verification, severity scoring — ADR-027), `src/detective/legal_gap_detector.py` (legal domain gap detection between written and applied law — ADR-007), `src/security/constitution.py` (epistemic moral compass client), `src/core/graph.py` (HybridGraphLayer + GATv2Conv), `src/core/providers.py` (Azure Foundry, vLLM, hybrid routing providers with circuit breaker auto-recovery — ADR-013), `src/core/reasoning_trace.py` + `src/core/trace_store.py` (reasoning trace capture with streaming pagination — ADR-014), `src/api/routes.py` (FastAPI endpoints + trace streaming + graph export + investigation endpoints + configurable CORS), `src/cli/main.py` (includes `extract-scenarios`, `ingest-epstein`, `network`, `investigate`, `audit-person` commands), `src/training/constitutional_warmup.py` (constitutional preference pair generation — full implementation), `src/training/train_multitask.py` (multi-task PyTorch trainer — ADR-028), `src/training/multitask_loss.py` (three-head loss computation), `src/training/multitask_dataset.py` (JSONL annotation loader + tokenizer), `src/training/multitask_config.py` (frozen training config), `src/training/generate_multitask_data.py` (synthetic data generator), `spaces/maninagarden/` (ZeroGPU-accelerated Gradio workbench: graph_client, graph_analytics, graph_features — ADR-017). Full test suite: 1097 tests passing.
...
**ADRs:** `docs/vault/decisions/` contains Architecture Decision Records (ADR-001 through ADR-028). Consult before making changes to the systems they cover.
...
| `DETECTIVE_KUZU_PATH` | kuzu backend | — | Path to Kuzu database directory |
...
data/training/             ← Training data (constitutional pairs, welfare data)
